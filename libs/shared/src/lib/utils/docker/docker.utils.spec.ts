import { DockerCommand, DockerRunOptions } from '../../types';
...
describe('DockerUtils', () => {
  describe('parseDockerCommand', () => {
    ...
    it.each([
      { command: 'docker run -p 80:80 alpine', publish: [{ protocol: 'tcp', source: '80', target: '80' }] },
      { command: 'docker run -p 8080:80 -p 443:443/udp alpine', publish: [{ protocol: 'tcp', source: '8080', target: '80' }, { protocol: 'udp', source: '443', target: '443' }] },
      { command: 'docker run --publish 80:80 alpine', publish: [{ protocol: 'tcp', source: '80', target: '80' }] },
      { command: 'docker run --publish 8080:80 --publish 443:443/udp alpine', publish: [{ protocol: 'tcp', source: '8080', target: '80' }, { protocol: 'udp', source: '443', target: '443' }] },
    ...
    ])('should return DockerRunOptions for command with remove', ({ command, securityOptions }) => {
    ...
    ])('should return DockerRunOptions for command with remove', ({ command, shmSize }) => {
    ...
    it.each([
      ...
      { command: 'docker run -v /etc/hosts:/etc/hosts:rw,Z alpine', volumes: [{ type: 'bind', host: '/etc/hosts', container: '/etc/hosts', options: ['rw', 'Z'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run -v data:/etc/hosts alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts' }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run -v data:/etc/hosts:ro alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['ro'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run -v data:/etc/hosts:ro,z alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['ro', 'z'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run -v data:/etc/hosts:ro,Z alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['ro', 'Z'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run -v data:/etc/hosts:rw alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['rw'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run -v data:/etc/hosts:rw,z alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['rw', 'z'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run -v data:/etc/hosts:rw,Z alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['rw', 'Z'] }] satisfies DockerRunOptions['volumes'] },
      ...
      { command: 'docker run --volume /etc/hosts:/etc/hosts:rw,Z alpine', volumes: [{ type: 'bind', host: '/etc/hosts', container: '/etc/hosts', options: ['rw', 'Z'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run --volume data:/etc/hosts alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts' }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run --volume data:/etc/hosts:ro alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['ro'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run --volume data:/etc/hosts:ro,z alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['ro', 'z'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run --volume data:/etc/hosts:ro,Z alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['ro', 'Z'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run --volume data:/etc/hosts:ro alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['ro'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run --volume data:/etc/hosts:rw alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['rw'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run --volume data:/etc/hosts:rw,z alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['rw', 'z'] }] satisfies DockerRunOptions['volumes'] },
      { command: 'docker run --volume data:/etc/hosts:rw,Z alpine', volumes: [{ type: 'volume', host: 'data', container: '/etc/hosts', options: ['rw', 'Z'] }] satisfies DockerRunOptions['volumes'] },
  ...
  describe('stringifyDockerCommand', () => {
    ...
    it('should return string for DockerCommand with volume', () => {
      const dockerRunCommand: DockerCommand = {
        type: 'run',
        image: 'alpine',
        tag: 'latest',
