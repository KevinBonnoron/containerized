...
@Component({
  selector: 'containerized-docker-images-view-table',
  standalone: true,
...
export class DockerImagesViewTableComponent extends AbstractUiDirective {
  readonly matSort = viewChild.required(MatSort);
  readonly matPaginator = viewChild.required(MatPaginator);

  readonly displayedColumns = ['select', 'id', 'labels', 'tags', 'used', 'actions'];
