...
const providers = [importProvidersFrom(FeatureDockerEventsDataAccessModule), importProvidersFrom(FeatureDockerSystemDataAccessModule)];

const canActivate = [ensureEnvironmentSelectedGuard];

export const dockerSystemRoutes: Routes = [
  {
    path: ':environmentId',
    canMatch: [isNumberMatcher('environmentId')],
    providers,
    canActivate,
    children: [
